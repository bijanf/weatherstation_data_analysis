...
class PotsdamDataFetcher:
    ...
    def _validate_data_coverage(
        ...
        if year == 2025 or coverage_percentage >= self.min_coverage:
            ...
            print(f"✅ {year}: {actual_days}/{expected_days} days ({coverage_percentage:.1f}%) {status}")
            return True
        else:
            ...
            try:
                ...
                if not data.empty and 'tmax' in data.columns:
                    ...
                    if year == 2025 or coverage_percentage >= self.min_coverage:
                        ...
                        status = "(partial year)" if year == 2025 else ""
                        print(f"✅ {year}: {actual_days}/{expected_days} days, max: {max_temp:.1f}°C ({coverage_percentage:.1f}%) {status}")
                    else:
                        print(f"❌ {year}: Insufficient coverage ({coverage_percentage:.1f}%)")
                else:
            ...
            try:
                ...
                if not data.empty and 'prcp' in data.columns:
                    ...
                    if year == 2025 or coverage_percentage >= self.min_coverage:
                        ...
                        status = "(partial year)" if year == 2025 else ""
                        print(f"✅ {year}: {actual_days}/{expected_days} days, total: {total:.1f}mm ({coverage_percentage:.1f}%) {status}")
                    else:
                        print(f"❌ {year}: Insufficient coverage ({coverage_percentage:.1f}%)")
                else:
