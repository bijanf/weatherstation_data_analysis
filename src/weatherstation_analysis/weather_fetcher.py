...
class WeatherDataFetcher:
    ...
    def __init__(self, city_name: str = "Potsdam", min_coverage: float = 80.0):
        ...
        if not self.city_info:
            ...
            if suggestions:
                print(f"❌ City '{city_name}' not found. Did you mean: {', '.join(suggestions)}?")
            else:
    ...
    def _validate_data_coverage(
        ...
        for col in required_columns:
            ...
            if year != 2025 and coverage_percentage < self.min_coverage:
                print(f"❌ {year}: Insufficient {col} coverage ({coverage_percentage:.1f}%)")
                return False
        ...
        for year in years_to_analyze:
            ...
            try:
                ...
                if not data.empty:
                    ...
                    if available_columns and self._validate_data_coverage(data, year, available_columns):
        ...
        for year in years_to_analyze:
            ...
            try:
                ...
                if not data.empty and {"tmax", "tmin"}.intersection(data.columns):
                    ...
                    temp_columns = [col for col in ["tmax", "tmin"] if col in data.columns]
