...
class DroughtPlotter:
    ...
    def plot_precipitation_deficit_timeseries(
        ...
        actual_prcp = deficit_data[deficit_data.columns[0]].values  # First column is precipitation

        # Top panel: Actual vs Normal precipitation
        ax1.bar(years, actual_prcp, color='steelblue', alpha=0.7, label='Actual Precipitation')
        ax1.axhline(baseline_mean, color='darkgreen', linestyle='--', linewidth=2,
                    label=f'Baseline Normal ({baseline_mean:.0f} mm)')

        ax1.set_ylabel('Annual Precipitation (mm)', fontsize=11, fontweight='bold')
        ax1.set_title(f'Precipitation Analysis - {station_name}',
        ...
        for idx, row in deficit_data.iterrows():
            ...
            if row['deficit_percent'] > 30 or row['deficit_percent'] < -30:
                ax1.text(year, prcp, f"{row['percent_of_normal']:.0f}%",
                        ha='center', va='bottom', fontsize=8, fontweight='bold')

        # Bottom panel: Deficit/Surplus
        colors = ['darkred' if x > 0 else 'darkgreen' for x in deficit_data['deficit_mm']]
        ...
        ax2.set_title('Precipitation Deficit (positive = deficit, negative = surplus)',
                      fontsize=11, fontweight='bold')
        ax2.grid(True, alpha=0.3)

        # Add cumulative deficit text
        cumulative_deficit = deficit_data['deficit_mm'].sum()
        ax2.text(0.02, 0.98, f'Cumulative Deficit: {cumulative_deficit:.0f} mm',
    ...
    def plot_spi_timeseries(
        ...
        ax.axhline(-1.0, color='orange', linestyle='--', linewidth=1, alpha=0.5, label='Moderate Drought')
        ax.axhline(-1.5, color='orangered', linestyle='--', linewidth=1, alpha=0.5, label='Severe Drought')
        ax.axhline(-2.0, color='darkred', linestyle='--', linewidth=1, alpha=0.5, label='Extreme Drought')
        ax.axhline(1.0, color='royalblue', linestyle='--', linewidth=1, alpha=0.5, label='Wet')

        # Highlight drought period
        if drought_period:
            start_year, end_year = drought_period
            ax.axvspan(pd.Timestamp(f'{start_year}-01-01'),
                      pd.Timestamp(f'{end_year}-12-31'),
                      alpha=0.2, color='red', label=f'Drought Period ({start_year}-{end_year})')

        ax.set_xlabel('Year', fontsize=11, fontweight='bold')
        ax.set_ylabel(f'SPI-{scale_months} (Standardized)', fontsize=11, fontweight='bold')
        ax.set_title(f'Standardized Precipitation Index (SPI-{scale_months}) - {station_name}',
                    fontsize=13, fontweight='bold', pad=15)
    ...
    def plot_multi_station_comparison(
        ...
        ax1.set_xlabel(metric_labels.get(metric, metric), fontsize=11, fontweight='bold')
        ...
        for i, (bar, value) in enumerate(zip(bars, values)):
            ...
            ax1.text(x_pos, i, f'{value:.1f}', va='center', ha=ha,
                    fontsize=9, fontweight='bold')

        # Right panel: Worst years comparison
        worst_years = [regional_results[s]['worst_year'] for s in stations]
        worst_deficits = [regional_results[s]['worst_deficit_percent'] for s in stations]

        scatter = ax2.scatter(worst_years, worst_deficits, s=200, alpha=0.6,
                            c=worst_deficits, cmap='YlOrRd', edgecolors='black', linewidth=1)

        # Add station labels
        for station, year, deficit in zip(stations, worst_years, worst_deficits):
            ax2.annotate(station, (year, deficit), fontsize=8,
    ...
    def plot_comprehensive_drought_dashboard(
        ...
        ax1.fill_between(years, baseline_mean, actual_prcp,
        ...
        colors = ['darkred' if x < 100 else 'darkgreen' for x in deficit_data['percent_of_normal']]
        ...
        for idx, row in deficit_data.iterrows():
            if row['percent_of_normal'] < 60:
                ax2.text(row['year'], row['percent_of_normal'], f"{row['percent_of_normal']:.0f}%",
        ...
        ax3.fill_between(years, 0, cumulative_deficit, where=(cumulative_deficit >= 0),
                        color='darkred', alpha=0.6, label='Cumulative Deficit')
        ...
        ax3.text(0.98, 0.98, f'Total: {final_cumulative:.0f} mm',
...
"""
        ...
        # Overall title
        fig.suptitle(f'Comprehensive Drought Analysis Dashboard - {station_name}',
