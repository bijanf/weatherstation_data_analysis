...
class WeatherPlotter:
    ...
    def _add_data_attribution(self, ax: plt.Axes, position: str = 'bottom_right') -> None:
        """
        ...
        """
        today = date.today()
        attribution_text = f'Data: Meteostat/DWD\\nUpdated: {today.strftime("%d.%m.%Y")}'
        ...
        ax.text(x, y, attribution_text, transform=ax.transAxes, 
                ha=ha, va=va, fontsize=10, color='#444444', alpha=0.8)
    ...
    def plot_annual_precipitation_extremes(
        ...
        ax1.scatter(years, max_precip, color=self.colors['primary'], 
        ...
        ax1.plot(trend_years, trend_line, color=self.colors['danger'], 
                linewidth=2, alpha=0.8)
        
        # Highlight extreme years
        top_5_idx = np.argsort(max_precip)[-5:]
        ax1.scatter(years[top_5_idx], max_precip[top_5_idx], 
                   color=self.colors['danger'], s=100, 
                   edgecolors='black', linewidth=2, zorder=5)
        
        # Annotate highest
        max_idx = np.argmax(max_precip)
        ax1.annotate(f'{max_precip[max_idx]:.1f}mm\\n({years[max_idx]})', 
        ...
        ax1.set_ylabel('Maximum Daily Precipitation (mm)', fontsize=14, fontweight='bold')
        ax1.set_title('Annual Maximum Daily Precipitation\\nPotsdam Säkularstation (1893-2024)', 
        ...
        ax2.scatter(return_periods, sorted_precip, color=self.colors['warning'], 
                   s=50, alpha=0.7, edgecolors='black', linewidth=0.5)
        ...
        ax2.plot(theoretical_rp, theoretical_precip, color=self.colors['danger'], 
                linewidth=2, label='Gumbel Distribution Fit', alpha=0.8)
        ...
        ax2.set_title('Return Period Analysis - Maximum Daily Precipitation', 
                     fontsize=16, fontweight='bold', pad=20)
        ...
        for rp in [2, 5, 10, 25, 50, 100]:
            if rp <= return_periods.max():
                ...
                ax2.text(rp, precip_val + 2, f'{rp}y', ha='center', va='bottom', 
    ...
    def plot_temperature_extremes_analysis(
        ...
        ax4.hist(max_temp, bins=20, alpha=0.7, color=self.colors['danger'], 
                edgecolor='black')
        ...
        ax4.set_title('Distribution of Annual Maximum Temperatures', 
                     fontsize=14, fontweight='bold')
        
        hot_extremes = (max_temp > 35).sum()
        ax4.text(0.95, 0.95, f'Years > 35°C: {hot_extremes}', 
                transform=ax4.transAxes, ha='right', va='top', fontsize=11, 
                bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
        self._setup_plot_style(ax4)
        
        plt.suptitle('Temperature Extremes Analysis - Potsdam Säkularstation (1893-2024)', 
    ...
    def plot_threshold_exceedance_analysis(
        ...
        ax1.scatter(years, heavy_rain_days, color=self.colors['primary'], s=40, alpha=0.7)
        z1 = np.polyfit(years, heavy_rain_days, 2)
        p1 = np.poly1d(z1)
        ax1.plot(trend_years, p1(trend_years), color=self.colors['danger'], 
                linewidth=2, alpha=0.8)
        ...
        ax2.scatter(years, extreme_rain_days, color=self.colors['warning'], s=40, alpha=0.7)
        z2 = np.polyfit(years, extreme_rain_days, 2)
        p2 = np.poly1d(z2)
        ax2.plot(trend_years, p2(trend_years), color=self.colors['danger'], 
                linewidth=2, alpha=0.8)
        ...
        p3 = np.poly1d(z3)
        ax3.plot(trend_years, p3(trend_years), color=self.colors['danger'], 
                linewidth=2, alpha=0.8)
        ax3.set_xlabel('Year', fontsize=12, fontweight='bold')
        ...
        p4 = np.poly1d(z4)
        ax4.plot(trend_years, p4(trend_years), color=self.colors['danger'], 
                linewidth=2, alpha=0.8)
        ...
        plt.suptitle('Threshold Exceedance Analysis - Potsdam Säkularstation (1893-2024)', 
                    fontsize=18, fontweight='bold', y=0.98)
    ...
    def plot_statistics_summary(
        ...
        box_colors = [self.colors['primary'], self.colors['danger'], 
                     self.colors['success'], self.colors['warning']]
        
        boxes = ax1.boxplot(data_to_plot, labels=labels, patch_artist=True, 
        ...
        corr_data = extremes_df[['max_precip', 'max_temp', 'min_temp', 'temp_range']].corr()
        ...
        ax2.set_xticklabels(['Max Precip', 'Max Temp', 'Min Temp', 'Temp Range'], 
                           rotation=45)
        ax2.set_yticklabels(['Max Precip', 'Max Temp', 'Min Temp', 'Temp Range'])
        ax2.set_title('Correlation Matrix - Extreme Values', fontsize=14, fontweight='bold')
        ...
        colors = [self.colors['success'] if trend >= 0 else self.colors['danger'] 
                 for trend in trends]
        ...
        ax3.set_title('Long-term Trends in Extreme Values', fontsize=14, fontweight='bold')
        self._setup_plot_style(ax3)
        ...
        ax4.bar(decade_counts.index, decade_counts.values, width=8, alpha=0.7, 
        ...
        plt.suptitle('Extreme Value Statistics Summary - Potsdam Säkularstation (1893-2024)', 
                    fontsize=18, fontweight='bold', y=0.98)
