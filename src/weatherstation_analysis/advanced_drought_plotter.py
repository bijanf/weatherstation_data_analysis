...
class AdvancedDroughtPlotter:
    ...
    def plot_return_period_analysis(
        ...
        ranked_droughts = ranked_droughts.sort_values(
            "deficit_percent", ascending=False
        ...
        deficit_theoretical = stats.gumbel_r.ppf(
            1 - 1 / rp_range, gumbel_params["location"], gumbel_params["scale"]
        )
        ax1.plot(
            rp_range,
            deficit_theoretical,
        ...
        colors = [
            (
                ...
                if y in (drought_highlight_years or [])
    ...
    def plot_compound_event_analysis(
        ...
        drought_mask = (annual_anomalies["year"] >= drought_period[0]) & (
        ...
        ax1.fill_between(
            [-4, -1], [1, 1], [4, 4], alpha=0.1, color=self.COLORS["compound"]
        )
        ax1.text(
            -2.5,
            2.5,
        ...
        ax1.set_title(
            "Compound Drought-Heat Event Analysis", fontweight="bold", fontsize=13
        )
        ...
        legend_elements = [
            Line2D(
                ...
                marker="o",
                color="w",
                markerfacecolor=self.COLORS["compound"],
                markersize=10,
            ...
            Line2D(
                ...
                marker="o",
                color="w",
                markerfacecolor=self.COLORS["drought_moderate"],
                markersize=10,
                label="Drought only",
            ),
            Line2D(
            ...
            Line2D(
                ...
                marker="o",
                color="w",
                markerfacecolor=self.COLORS["normal"],
                markersize=10,
                label="Normal",
            ),
        ]
...
"""
        ...
        # Mark compound years
        compound_years = annual_anomalies[annual_anomalies["is_compound"]][
            "year"
        ].values
        ...
        ax3.set_title(
            "Precipitation Deficit and Temperature Anomaly Time Series",
            fontweight="bold",
            fontsize=13,
        )
        ...
        plt.suptitle(
            f"Compound Drought-Heat Event Analysis - {station_name}",
            fontsize=15,
            fontweight="bold",
            y=1.02,
        )
    ...
    def plot_multi_station_drought_map(
        ...
        ax2.set_xlabel("Precipitation Deficit (%)", fontweight="bold")
        ax2.set_title(
            "Station Ranking by Drought Severity", fontweight="bold", fontsize=13
        )
        ax2.grid(True, alpha=0.3, axis="x")
        ...
        for i, v in enumerate(sorted_values):
            ax2.text(
                v + 0.5, i, f"{v:.1f}%", va="center", fontweight="bold", fontsize=9
            )
    ...
    def plot_dsa_analysis(
        ...
        ax1.fill_between(
            dates,
            0,
            dsa_timeseries["area_fraction"] * 100,
            color=self.COLORS["drought_moderate"],
            alpha=0.7,
        )
        ax1.set_ylabel("Area Coverage (%)", fontweight="bold")
        ax1.set_title(
            "Drought Area Coverage (% of stations in drought)", fontweight="bold"
        )
        ax1.set_ylim(0, 100)
        ...
        ax2.fill_between(
            dates,
            0,
            dsa_timeseries["mean_severity"],
            color=self.COLORS["drought_severe"],
            alpha=0.7,
        )
        ax2.set_ylabel("Mean Severity (|SPI|)", fontweight="bold")
        ax2.set_title(
            "Drought Severity (mean absolute SPI of affected stations)",
            fontweight="bold",
        )
        ax2.axhline(
            1.5, color="red", linestyle="--", alpha=0.5, label="Severe threshold"
        )
        ax2.axhline(
            2.0, color="darkred", linestyle="--", alpha=0.5, label="Extreme threshold"
        )
        ax2.legend()
    ...
    def plot_regime_shift_analysis(
        ...
        means = [
            change_point_results["mean_before"],
            change_point_results["mean_after"],
        ]
        ...
        ax2.set_title(
            "Precipitation Before vs After Change Point", fontweight="bold", fontsize=13
        )
        ...
        ax3.plot(
            moving_stats["year"],
            moving_stats["moving_mean"],
            "r-",
            linewidth=2.5,
            label="10-year moving mean",
        )
        ax3.fill_between(
            moving_stats["year"],
        ...
        ax4.set_ylabel(
            "Mean Precipitation (mm)", fontweight="bold", color=self.COLORS["primary"]
        )
        ...
        ax4.set_title(
            "Decadal Precipitation and Drought Frequency",
            fontweight="bold",
            fontsize=13,
        )
        ...
        plt.suptitle(
            f"Precipitation Regime Analysis - {station_name}",
            fontsize=15,
            fontweight="bold",
            y=1.02,
        )
    ...
    def plot_historical_context(
        ...
        ax2.axvline(
            ...
            linewidth=3,
            linestyle="--",
            label=f"Drought mean: {mean_drought:.0f} mm",
        )
        ax2.axvline(
            baseline_mean,
            color="green",
        ...
        ax3.set_title(
            "Drought Severity Percentile Ranking", fontweight="bold", fontsize=13
        )
...
"""
        ax3.text(50, -0.3, context_text, ha="center", fontsize=10, style="italic")
...
"""
    ...
    def plot_all_stations_spi_heatmap(
        ...
        ax.set_title(
            "Multi-Station SPI-12 Heatmap (Blue = Wet, Red = Drought)",
            fontweight="bold",
            fontsize=13,
        )
        ...
        if start_idx and end_idx:
            ...
            ax.text(
                start_idx, -0.7, f"{drought_period[0]}", ha="center", fontweight="bold"
            )
            ax.text(
                end_idx, -0.7, f"{drought_period[1]}", ha="center", fontweight="bold"
            )
