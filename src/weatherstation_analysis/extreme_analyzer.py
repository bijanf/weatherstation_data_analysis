...
class ExtremeValueAnalyzer:
    ...
    def analyze_annual_extremes(self, exclude_current_year: bool = True) -> pd.DataFrame:
        """
    ...
    def detect_changepoints(self, variable: str, method: str = 'pettitt') -> Dict[str, any]:
        """
        ...
        if method == 'pettitt':
            ...
            for i in range(n):
                K[i] = np.sum(np.sign(values[i] - values[:i])) + np.sum(np.sign(values[i] - values[i+1:]))
