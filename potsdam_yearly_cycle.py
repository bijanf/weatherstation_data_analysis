#!/usr/bin/env python3
"""
Yearly-cycle climatology chart (FU-Berlin "Klimadiagramm" style).
...
* red bar / up-triangle   -> day warmer than the climatological daily maximum
* blue bar / down-triangle -> day colder than the climatological daily maximum

Behind the bars, nested grey bands show the 5/10/25/75/90/95 percentile
distribution of the reference period, and a table underneath lists the monthly
mean daily maximum and its anomaly vs. the reference period.
...
def monthly_table(series: pd.Series, year: int, ref_start: int, ref_end: int) -> dict:
    ...
    compared against a full reference month. The annual ('Jahr') anomaly follows
    the FU-Berlin convention and compares the year-to-date mean against the
    *full-year* reference climatology -- so it reflects the seasonal sampling of
    the elapsed part of the year, not a same-window anomaly.
    """
    ...
    # 'Jahr' column: year-to-date mean vs the full-year reference climatology
    # (FU-Berlin convention; intentionally not a same-period anomaly).
...
def make_plot(key: str, year: int | None = None) -> str:
    ...
    # Percentile labels just inside both margins (as in the FU-Berlin chart)
    for p in PCT_LEVELS:
    ...
    # Titles / credits
    fig.text((PLOT_LEFT + PLOT_RIGHT) / 2, 0.955,
             f"Tagesmaxima des Jahres für {year} im Vergleich zur "
             f"Referenzperiode {REF_START} - {REF_END}",
             ha="center", fontsize=13, fontweight="bold")
    fig.text(PLOT_LEFT, 0.925, station["label"], ha="left", fontsize=11)
    fig.text(0.985, 0.975, f"Erstellt am {today.strftime('%d.%m.%Y')}",
             ha="right", fontsize=9, color="#555555")
    fig.text(0.985, 0.925,
             "Daten: Meteostat / DWD", ha="right", fontsize=9, color="#555555")

    # Legend
    handles = [
        Line2D([0], [0], marker="^", color="w", markerfacecolor=RED,
               markersize=9, label="Temperatur > klimatolog. Tagesmaxima"),
        Line2D([0], [0], marker="v", color="w", markerfacecolor=BLUE,
               markersize=9, label="Temperatur < klimatolog. Tagesmaxima"),
        Line2D([0], [0], color=LINE_COLOR, linewidth=1.3,
               label=f"klimatolog. Tagesmaxima {REF_START} - {REF_END}"),
        Line2D([0], [0], marker="s", color="w", markerfacecolor="#c9c9c9",
               markeredgecolor="none", markersize=11,
               label="Verteilung der Tagesmaxima"),
    ]
...
def _draw_table(fig, table: dict) -> None:
    """Render the Monatsmittel / Abweichung rows beneath the axes (figure coords)."""
    # Row labels at the far left, right-aligned so they end before the plot box
    fig.text(PLOT_LEFT - 0.008, Y_MEAN, "Monatsmittel [°C]", ha="right",
             va="center", fontsize=9)
    fig.text(PLOT_LEFT - 0.008, Y_ANOM, "Abweichung [°C]", ha="right",
             va="center", fontsize=9)
    ...
    # 'Jahr' column header (aligned with the month names) + its values
    fig.text(JAHR_X, Y_HEADER, "Jahr", ha="center", va="center", fontsize=10,
             fontweight="bold")
